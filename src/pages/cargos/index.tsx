...
import { getCargos, deleteCargoById } from 'apiSdk/cargos';
import { CargoInterface } from 'interfaces/cargo';

type ColumnType = ColumnDef<CargoInterface, unknown>;
...
export function CargoListPage(props: CargoListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getCargos({
        relations: ['organization', 'user', 'booking.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['cargo_name.contains', 'cargo_type.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CargoInterface>>(
    () => `/cargos?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CargoInterface) => {
    if (hasAccess('cargo', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/cargos/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'cargo_name', header: 'Cargo Name', accessorKey: 'cargo_name' },
    { id: 'cargo_weight', header: 'cargo_weight', accessorKey: 'cargo_weight' },
    { id: 'cargo_volume', header: 'cargo_volume', accessorKey: 'cargo_volume' },
    { id: 'cargo_type', header: 'Cargo Type', accessorKey: 'cargo_type' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/cargos/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('cargo', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/cargos/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('cargo', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="cargo" />
        </Flex>

        {hasAccess('cargo', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/cargos/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
