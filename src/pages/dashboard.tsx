...
import { getCargos } from 'apiSdk/cargos';
import { CargoInterface } from 'interfaces/cargo';
import { getBusinessAdministrators } from 'apiSdk/business-administrators';
...
import { BusinessAdministratorListPage as ShowCaseList } from 'pages/business-administrators';

function DashboardPage() {
  const { session } = useSession();

  const cargoStat = useSWR<PaginatedInterface<CargoInterface>>(
    () => `/cargos`,
    () => getCargos({ limit: 1, offset: 0 }),
  );

  const businessAdministratorStat = useSWR<PaginatedInterface<BusinessAdministratorInterface>>(
    () => `/business-administrators`,
    () => getBusinessAdministrators({ limit: 1, offset: 0 }),
  );

  const driverStat = useSWR<PaginatedInterface<DriverInterface>>(
    () => `/drivers`,
    () => getDrivers({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Cargos',
      result: cargoStat?.data?.totalCount,
      isLoading: cargoStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Business Administrators',
      result: businessAdministratorStat?.data?.totalCount,
      isLoading: businessAdministratorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Drivers',
      result: driverStat?.data?.totalCount,
      isLoading: driverStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Business Administrators
                </Text>
                <NextLink href={`business-administrators`} passHref legacyBehavior>
                  <Button
                  ...
                  >
                    See all Business Administrators
                  </Button>
                </NextLink>
              </Flex>
