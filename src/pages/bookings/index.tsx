  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.booking_date ? format(parseISO(record?.booking_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.expected_delivery_date
          ? format(parseISO(record?.expected_delivery_date as unknown as string), 'dd-MM-yyyy')
          : '',
    },
    ...
    hasAccess('cargo', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'cargo',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/cargos/view/${record.cargo?.id}`}>
              {record.cargo?.cargo_name}
            </Link>
          ),
        }
      : null,
