...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'cargo.update': {
    roles: ['driver'],
    key: 'cargo-assigned-to-driver',
    tenantPath: ['organization', 'cargo'],
    userPath: [],
  },
  'booking.update': {
    roles: ['shipping-agent'],
    key: 'booking-status-updated',
    tenantPath: ['organization', 'cargo', 'booking'],
    userPath: [],
  },
};

const ownerRoles: string[] = ['platform-owner'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['platform-owner', 'business-administrator', 'driver', 'shipping-agent'];
